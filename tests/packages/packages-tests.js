require(
    {
      ...
      paths: {'alpha/replace': 'replace'},
      packages: [
        {name: 'alpha', location: 'pkgs/alpha'},
        {name: 'beta', location: 'pkgs/beta/0.2/scripts', main: 'beta'},
        {name: 'dojox/chair', location: 'pkgs/dojox/chair'},
        {name: 'dojox/table', location: 'pkgs/dojox/table', main: 'table'},
    ...
    function(
        require, alpha, replace, beta, util, bar, baz, foo, second, chair,
        table, door, pane, window, legs, funky, mypack) {
      var dataUrl = require.toUrl('foo/../data.html'),
          dataUrl2 = require.toUrl('foo/data.html');
      doh.register('packages', [function packages(t) {
                     ...
                     t.is(new mypack.Student() instanceof mypack.Person, true);
                   }]);
      doh.run();
    });
